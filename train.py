...
from bluntools.layers import weights_init, active_flatten, avg_class_weight, get_statistic, pre_visdom, \
    DiceLoss, NLLLoss, multi_size, get_border_weight
...
parser = argparse.ArgumentParser(description='Corneal Limbus Auto Measurement')
...
def main():
    ...
    # Dataset loader
    # --------------------------------------------------------------------------------------------------------
    training_data_loader = DataLoader(dataset=CornealLimbusDataset(mode='train'),
                                      num_workers=conf.threads, batch_size=conf.batch_size, shuffle=True)

    test_data_loader = DataLoader(dataset=CornealLimbusDataset(mode='test'),
                                  num_workers=conf.threads, batch_size=conf.batch_size, shuffle=True)
    ...
    def train():
        ...
        for partial_epoch, (image, label, label_prev) in enumerate(training_data_loader, 1):
            image = Variable(image).float().cuda()
            label_prev = Variable(label_prev).float().cuda()

            outputs, (targets, weights) = model(image, label_prev), multi_size(label, size=conf.loss_size)  # 2D
        ...
        print('===> Training step {} ({}/{})\t'.format(i, i // conf.augment_size + 1, conf.epochs),
              'Loss: {:.5f}   Accuracy: {:.5f}  |   Mean IOU: {:.5f}'.format(*print_format))
    ...
    def validate():
        ...
        for partial_epoch, (image, label, label_prev) in enumerate(test_data_loader, 1):
            image = Variable(image, volatile=True).float().cuda()
            label_prev = Variable(label_prev, volatile=True).float().cuda()
